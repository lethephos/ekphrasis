...
export function createRuntime({ env = process.env, fetchImpl = fetch } = {}) {
  ...
  const visualSearch = env.QDRANT_URL && env.QDRANT_API_KEY && env.QDRANT_COLLECTION
    ...
    : null;
  const visual = env.CLIP_EMBEDDING_URL && visualSearch
    ? createVisualResolver({
        embed: async (bytes) => {
          const response = await fetchImpl(env.CLIP_EMBEDDING_URL, { method: 'POST', headers: { 'content-type': 'application/octet-stream' }, body: Buffer.from(bytes) });
          if (!response.ok) throw new Error('CLIP_UNAVAILABLE');
          const body = await response.json();
          if (!Array.isArray(body.embedding)) throw new Error('CLIP_MALFORMED_RESPONSE');
