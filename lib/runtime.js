...
import { createWikipediaClient } from './enrichment/wikipedia.js';
import { createWikidataClient } from './enrichment/wikidata.js';
import { createContextEnricher } from './enrichment/context.js';
...
export function createRuntime({ env = process.env, fetchImpl = fetch, clipPipelineFactory, clipImageLoader } = {}) {
  ...
  const wikipedia = createWikipediaClient({ fetchImpl });
  const wikidata = createWikidataClient({ fetchImpl });
  const context = createContextEnricher({ wikipedia, wikidata });
  ...
  return {
    ...
    gate: decideMatch,
    visual,
    enrich: async (artwork) => ({
      ...(await enrichStyle(artwork, createStyleSource(fetchImpl))),
      ...(await context.enrich(artwork)),
    }),
  };
}
