...
export function createIdentifyPipeline(deps = {}) {
  return {
    async identify(bytes, { clientKey } = {}) {
      ...
      let visual = null;
      if (decision.useVisualFallback && deps.visual) visual = await deps.visual.resolve(normalized.bytes, decision);
      if (visual?.candidate) {
        const visualCandidate = visual.candidate;
        const visualScore = deps.score(query, visualCandidate);
        const visualDecision = deps.gate({ score: Math.max(visualScore.score, visual.similarity ?? 0), evidence: visualScore.evidence });
