  ...
  return {
    async identify(bytes, { clientKey } = {}) {
      ...
      if (decision.useVisualFallback && deps.visual) {
        const visual = await deps.visual.resolve(normalized.bytes, { ...decision, isCompatible: (candidate) => { const scored = deps.score(query, candidate); return !Object.values(scored.evidence ?? {}).includes('MISMATCH'); } });
        if (visual?.candidate) {
