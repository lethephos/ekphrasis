...
export function createIdentifyPipeline(deps = {}) {
  return {
    async identify(bytes, { clientKey } = {}) {
      ...
      let decision = best ? deps.gate(best) : { status: 'no_match', confidence: 'low', useVisualFallback: true };
      if ((!best || decision.useVisualFallback) && deps.visual) {
        const visual = await deps.visual.resolve(normalized.bytes, {
          ...decision,
          isCompatible: (candidate) => {
            if (!best) return true;
            const scoredCandidate = deps.score(query, candidate);
            return !Object.values(scoredCandidate.evidence ?? {}).includes('MISMATCH');
          },
        });
        if (visual?.candidate) {
      ...
      let enrichment = { style: best.candidate.style ?? null, styleSource: best.candidate.style ? 'museum' : null, context: null, detail: null };
      if (deps.enrich) { try { enrichment = { ...enrichment, ...(await deps.enrich(best.candidate)) }; } catch {} }
      ...
      const result = buildIdentificationResult({
        ...
        artwork: {
          ...
          styleSource: enrichment.styleSource,
        ...
        detail: enrichment.detail ?? null,
        related_reading: enrichment.related_reading ?? [],
