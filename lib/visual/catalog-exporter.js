  ...
  return {
    ...
    async exportSmithsonian({ query = 'painting', limit = 100, apiKey } = {}) {
      ...
      while (records.length < limit) {
        ...
        url.searchParams.set('rows', String(Math.min(limit - records.length, 100)));
        url.searchParams.set('start', String(start));
        const body = await request(url);
        const page = (body.response?.rows ?? []).map(normalizeSmithsonian).filter((r) => r.imageUrl && r.objectId);
        const needed = limit - records.length;
        records.push(...page.slice(0, needed));
        if (page.length === 0 || page.length < Math.min(needed, 100)) break;
        start += page.length;
      }
      return records;
    },
