...
function normalizeSearchResults(payload) {
  return (Array.isArray(payload) ? payload : payload?.result?.points ?? payload?.result?.result ?? payload?.result ?? []).map((item) => ({
...
export function createQdrantClient({ search, endpoint, apiKey, collection = process.env.QDRANT_COLLECTION, fetchImpl = fetch, timeoutMs = 6000 } = {}) {
  ...
  return {
    async search(vector) {
      ...
      try {
        const response = await fetchImpl(`${endpoint.replace(/\/$/, '')}/collections/${encodeURIComponent(collection)}/points/query`, {
          method: 'POST',
          headers: { 'content-type': 'application/json', 'api-key': apiKey },
          body: JSON.stringify({ query: vector, limit: 10, with_payload: true }),
...
export function createQdrantAdminClient({ endpoint, apiKey, fetchImpl = fetch, timeoutMs = 6000 } = {}) {
  ...
  return {
    ...
    switchAlias: async (alias, collection) => {
      const aliases = await request('/aliases', 'GET');
      const exists = (aliases?.result?.aliases ?? []).some((item) => item.alias_name === alias);
      const actions = exists ? [
        { delete_alias: { alias_name: alias } },
        { create_alias: { collection_name: collection, alias_name: alias } },
      ] : [{ create_alias: { collection_name: collection, alias_name: alias } }];
      return request('/collections/aliases', 'POST', { actions });
    },
  };
}
