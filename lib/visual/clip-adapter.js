...
export function createProductionClipEmbedder({ model = DEFAULT_MODEL, pipelineFactory, imageLoader } = {}) {
  const createPipeline = pipelineFactory ?? (async (task, selectedModel) => {
    const { pipeline } = await import('@huggingface/transformers');
    return pipeline(task, selectedModel);
  });
  const loadImage = imageLoader ?? (async (bytes) => {
    const { RawImage } = await import('@huggingface/transformers');
    return RawImage.read(new Blob([bytes]));
  });
  return createClipEmbedder({
    model,
    pipelineFactory: (selectedModel) => createPipeline('image-feature-extraction', selectedModel),
